...
class Ada_Weak_Classifier(Weak_Classifier):
	...
	def calc_error(self, weights, labels):
		minimum = min(self.activations)
		maximum = max(self.activations)
		thresholds = np.linspace(minimum, maximum, self.num_bins)
		changed_labels = (labels[:] > 0)
		matrix = ((thresholds[:,None] <= self.activations) != changed_labels[None,:])
		error = matrix @ weights
		index = error > 0.5
		polarity = np.ones(len(error))
		error[index] = 1 - error[index]
		polarity[index] = -1
		final_index = np.argmin(error)
		return error[final_index], thresholds[final_index], polarity[final_index]
...
class Real_Weak_Classifier(Weak_Classifier):
	...
	def calc_error(self, weights, labels):
		minimum = min(self.activations)
		maximum = max(self.activations)
		self.thresholds = np.linspace(minimum, maximum, self.num_bins)
		assigned_values = np.digitize(self.activations, self.thresholds, right = True)
		unique_elements = np.unique(assigned_values)
		bins = [np.where(assigned_values == i) for i in unique_elements]
		self.bin_pqs = np.zeros((2,self.num_bins))
		for i in range(len(unique_elements)):
			p,q =10e-6,10e-6
			for j in bins[i][0]:
				p += weights[j] if labels[j] == 1 else 0
				q += weights[j] if labels[j] == -1 else 0
			self.bin_pqs[0,i] = p
			self.bin_pqs[1,i] = q
		self.bin_pqs[self.bin_pqs == 0] = 10e-6
		return
